...
# ttcontour.py
#
# Copyright © 2004-2012, 2014-2017 Monotype Imaging Inc. All Rights Reserved.
#
...
from fontio3.fontmath import bspline, line, mathutilities, point
from fontio3.glyf import ttbounds
...
class TTContour(list, metaclass=seqmeta.FontDataMetaclass):
    """
    Objects representing single TrueType contours. These are lists of TTPoint
    objects.
    
    Note that TTContours do not have fromwalker() or buildBinary() methods;
    those functions are handled at a higher level, given the convoluted nature
    of the TrueType 'glyf' table.
    ...
    def bounds(self):
        """
        Returns a TTBounds with the extrema based on the curves; this is
        ...
        return (r if r is None else ttbounds.TTBounds(*r.asList()))
    ...
    def extrema(self, excludeOffCurve=False):
        """
        Returns a TTBounds with the extrema based on the actual coordinates;
    ...
    def intersects(self, other):
        ...
        >>> shifted = TTContour([p.moved(20, 20) for p in _testingValues[0]])
        >>> _testingValues[0].pprint()
        Point 0: (20, 10), on-curve
        Point 1: (20, 490), on-curve
        Point 2: (380, 490), on-curve
    ...
    def normalized(self, **kwArgs):
        ...
        There are also optional effects controlled via these keyword arguments:
            ...
            noElision           Default False. If True, then coincident points
                                won't be combined into a single point.
        ...
        >>> c.normalized(noElision=True).pprint()
        Point 0: (100, 900), on-curve
        Point 1: (100, 901), off-curve
        Point 2: (100, 900), on-curve
        Point 3: (900, 900), on-curve
        Point 4: (900, 100), on-curve
        Point 5: (100, 100), on-curve
        
        >>> c = TTContour([
        ...   P(61, -30, onCurve=False),
        ...   P(61, -28)])
        >>> c.normalized().pprint()
        """
        ...
        noElision = kwArgs.get('noElision', False)
        ...
        for thisPt, nextPt, afterNextPt in zip(self, it, it2):
            ...
            if (not noElision) and (tuple(thisPt) == tuple(nextPt)):
                thisPtIndex += 1
                continue
...
if __debug__:
    ...
    _testingValues = (
        ...
        TTContour(pv[12:16]),
        TTContour(pv[0:4]),
        TTContour((pv[8],) + pv[8:12]),
        TTContour(pv[4:6] + (pv[4],) + pv[6:8]),
        TTContour((pv[1],) + pv[4:6] + (pv[2],) + pv[6:8]),
        TTContour((pv[4],) + (pv[6],) + (pv[5],) + (pv[7],))  # self-intersects
        )
