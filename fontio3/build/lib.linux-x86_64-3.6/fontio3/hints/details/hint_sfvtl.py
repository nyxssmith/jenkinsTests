...
def hint_SFVTL(self, **kwArgs):
    """
    SFVTL: Set freedom vector to line, opcodes 0x08-0x09
    
    >>> logger = utilities.makeDoctestLogger("SFVTL_test")
    >>> _popSync, _testingState = _testFuncs()
    >>> h = _testingState(14, 3, 11)
    >>> hint_SFVTL(h, logger=logger)
    ...
    >>> hint_SFVTL(h, logger=logger)
    SFVTL_test - CRITICAL - Stack underflow in test (PC 1).
    """
    ...
    if self._zoneCheck("SFVTL", (1, 2), logger):
    ...
    if fatObj is not None:
        fatObj.notePop('pointIndex', 'SFVTL')
        fatObj.notePop('pointIndex', 'SFVTL')
