...
class Hint_DELTAG_1(set, metaclass=setmeta.FontDataMetaclass):
    """
    Objects representing Spark DELTAG1 opcodes. These are sets of DeltaSpec
    ...
    def __str__(self):
        ...
        >>> obj = Hint_DELTAG_1({
        ...   hint_deltaspec.DeltaSpec(12, None, -0.125),
        ...   hint_deltaspec.DeltaSpec(15, None, 0.25)})
        >>> print(obj)
        DELTAG: -1/8@12, 1/4@15
        """
    ...
    def buildBinary(self, w, **kwArgs):
        ...
        band = 1  # hardwired for Hint_DELTAG_1
    ...
    def decompose(self):
        """
        Returns a list of Hint_DELTAG_1 objects matching self, but where each
    ...
    def fromvalidatedwalker(cls, w, **kwArgs):
        """
        Returns a new Hint_DELTAG_1 from the specified walker, with validation.
        ...
        >>> fvb = Hint_DELTAG_1.fromvalidatedbytes
        >>> bs = utilities.fromhex("A1 02 37 69")
        >>> d = {'deltaBase': 9, 'deltaShift': 3, 'logger': logger}
        >>> print((fvb(bs, **d)))
        test.DELTAG1 - DEBUG - Remaining walker bytes: 4
        DELTAG: -1/8@12, 1/4@15
        
        >>> fvb(bs[:-1], **d)
        test.DELTAG1 - DEBUG - Remaining walker bytes: 3
        test.DELTAG1 - ERROR - Insufficient bytes.
        """
        
        logger = kwArgs.pop('logger', None)
        
        if logger is None:
            logger = logging.getLogger().getChild('DELTAG1')
        else:
            logger = logger.getChild('DELTAG1')
        ...
        if op != 0xA1:
            logger.error((
              'V0002',
              (op,),
              "Was expecting opcode 0xA1, but got 0x%02X instead"))
    ...
    def fromwalker(cls, w, **kwArgs):
        """
        Returns a new Hint_DELTAG_1 from the specified walker.
