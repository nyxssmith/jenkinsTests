...
class GlyphTupleDict(dict, metaclass=mapmeta.FontDataMetaclass):
    ...
    def glyphsRenumbered(self, oldToNew, **kwArgs):
        """
        Do the usual glyph renumbering, but then prune if the renumbering left
        any elements with fewer entries than before.
        
        >>> otn = {9: 9, 23: 23, 30: 30, 97: 97, 98: 98}
        >>> _testingValues[2].glyphsRenumbered(otn, keepMissing=False)
        GlyphTupleDict({})
        >>> del otn[30]
        >>> otn[15] = 15
        >>> _testingValues[2].glyphsRenumbered(otn, keepMissing=False)
        GlyphTupleDict({GlyphTupleInput((9, 15, 23)): GlyphTupleOutput((97, None, None))})
        """
        
        if not self:
            return self.__copy__()
        
        priorLen = len(next(iter(self)))
        r = mapmeta.M_glyphsRenumbered(self, oldToNew, **kwArgs)
        toDel = set()
        
        for gti, gto in r.items():
            if len(gti) != priorLen or len(gto) != priorLen:
                toDel.add(gti)
...
if __debug__:
    from fontio3.morx import glyphtuple
