...
from fontio3.HVAR import deltasetindexmap, livingdeltas_dict  # intentional from HVAR
...
def _validate(obj, **kwArgs):
    """
    isValid() validation for the top-level VVAR
    """
...
class VVAR(object, metaclass=simplemeta.FontDataMetaclass):
    """
    Objects representing a VVAR table. These are simple objects that contain
    ...
    Vertical Origin Deltas: (no data)
    """
    ...
    attrSpec = dict(
        ...
        verticalorigins = dict(
            attr_followsprotocol = True,
            attr_initfunc = lambda: None,
            attr_label = "Vertical Origin Deltas",
            attr_renumberdirect = True),
        )
    ...
    def buildBinary(self, w, **kwArgs):
        """
        Adds the binary data for the VVAR object to the specified LinkedWriter.
        ...
        >>> utilities.hexdump(obj.binaryString(axisOrder=('wght', 'wdth')))
               0 | 0001 0000 0000 0018  0000 0040 0000 0000 |...........@....|
              10 | 0000 0000 0000 0000  0001 0000 0018 0001 |................|
              20 | 0000 000C 0002 0001  0001 0000 FF4C 0028 |.............L.(|
              30 | 0002 0001 D000 0000  4000 C000 1000 3000 |........@.....0.|
              40 | 0010 0002 0000 0001                      |........        |
        """
        ...
        if self.leftsidebearings:
            lsbVals = [t[1] for t in sorted(self.leftsidbearings.items())]
        else:
            lsbVals = []
        ...
        if self.verticalorigins:
            vorgVals = [t[1] for t in sorted(self.verticalorigins.items())]
        else:
            vorgVals = []

        # convert Living Deltas back to a binary string + LivingDelta-to-DeltaSetIndexMap
        valIter = itertools.chain(advVals, lsbVals, rsbVals, vorgVals)
        ...
        if self.verticalorigins:
            count = len(self.verticalorigins)
            stake_VOM = w.getNewStake()
            w.addUnresolvedOffset("L", stakeValue, stake_VOM)
            vorgMap = {g: LDtoDSMap[self.verticalorigins[g]] for g in range(count)}
            vorgDSMap = deltasetindexmap.DeltaSetIndexMap(vorgMap)
        else:
            w.add("L", 0)
        ...
        if self.verticalorigins:
            w.stakeCurrentWithValue(stake_VOM)
            vorgDSMap.buildBinary(w, **kwArgs)


    @classmethod
    def fromvalidatedwalker(cls, w, **kwArgs):
        """
        Creates and returns a new VVAR object from the specified walker, doing
        validation.
        ...
        >>> s = (
        ...  "0001 0000 0000 0018  0000 0040 0000 0000"
        ...  "0000 0000 0000 0000  0001 0000 0018 0001"
        ...  "0000 000C 0002 0001  0001 0000 FF4C 0028"
        ...  "0002 0001 D000 0000  4000 C000 1000 3000"
        ...  "0000 0002 0001")
        >>> b = utilities.fromhex(s)
        >>> fvb = VVAR.fromvalidatedbytes
        >>> obj = fvb(b, logger=logger, axisOrder=('wght','wdth'), fontGlyphCount=2)
        test.VVAR - DEBUG - Walker has 70 remaining bytes.
        test.VVAR.version - DEBUG - Walker has 70 remaining bytes.
        test.VVAR.IVS - DEBUG - Walker has 46 remaining bytes.
        test.VVAR.IVS - INFO - Format 1
        test.VVAR.IVS - DEBUG - Data count is 1
        test.VVAR.IVS - DEBUG - Axis count is 2
        test.VVAR.IVS - DEBUG - Region count is 1
        test.VVAR.IVS - DEBUG - Delta (0, 0)
        test.VVAR.IVS - DEBUG - Delta (0, 1)
        test.VVAR - DEBUG - Advance Width Delta-Set Index Map present
        test.VVAR.deltasetindexmap - DEBUG - Walker has 6 remaining bytes.
        test.VVAR.deltasetindexmap - DEBUG - entryFormat = 0x0
        test.VVAR.deltasetindexmap - DEBUG - mapCount = 2
        test.VVAR.deltasetindexmap - DEBUG - innerBitCount = 1
        test.VVAR.deltasetindexmap - DEBUG - mapEntrySize = 1 bytes
        test.VVAR.deltasetindexmap - DEBUG - Entry 0 (0, 0); rawData = 0x0
        test.VVAR.deltasetindexmap - DEBUG - Entry 1 (0, 1); rawData = 0x1
        test.VVAR - DEBUG - Advances glyph 0 DeltaSetIndex (0, 0)
        test.VVAR - DEBUG - Advances glyph 1 DeltaSetIndex (0, 1)
        ...
        if logger is None:
            logger = logging.getLogger().getChild('VVAR')
        else:
            logger = logger.getChild('VVAR')
        ...
        oIVS, oAWM, oLSBM, oRSBM, oVOM = w.unpack("5L")
        ...
        if oRSBM:
            ...
            gidToDSMap = DSIMfvw(wRsbm, logger=logger, **kwArgs)
            defDSVal = sorted(gidToDSMap.items())[-1][1]
            lds = {g:ivs[gidToDSMap.get(g, defDSVal)] for g in range(fontGlyphCount)}
            r.rightsidebearings = livingdeltas_dict.LivingDeltasDict(lds)

        if oVOM:
            logger.debug(('Vxxxx', (), "VORG Delta-Set Index Map present"))
            wVom = w.subWalker(oVOM)
            gidToDSMap = DSIMfvw(wVom, logger=logger, **kwArgs)
            defDSVal = sorted(gidToDSMap.items())[-1][1]
            lds = {g:ivs[gidToDSMap.get(g, defDSVal)] for g in range(fontGlyphCount)}
            r.verticalorigins = livingdeltas_dict.LivingDeltasDict(lds)
    ...
    def fromwalker(cls, w, **kwArgs):
        """
        Creates and returns a new VVAR object from the specified walker.
        ...
        if oRSBM:
            wRsbm = w.subWalker(oRSBM)
            gidToDSMap = DSIMfw(wRsbm, **kwArgs)
            defDSVal = sorted(gidToDSMap.items())[-1][1]
            lds = {g:ivs[gidToDSMap.get(g, defDSVal)] for g in range(fontGlyphCount)}
            r.rightsidebearings = livingdeltas_dict.LivingDeltasDict(lds)
