...
"""
Definitions for OpenType VariationIndex records. These are similar to Device objects, but instead of storing device tweaks, they store LivingDeltas objects.
"""
...
from fontio3.opentype import variationindex
...
class Device(dict, metaclass=mapmeta.FontDataMetaclass):
    ...
    def fromvalidatedwalker(cls, w, **kwArgs):
        ...
        test.device - DEBUG - Start size is 12 and end size is 18.
        test.device - DEBUG - Format is 1.
        ...
        test.device - DEBUG - Start size is 12 and end size is 20.
        test.device - DEBUG - Format is 2.
        test.device - DEBUG - Data are (48624, 32, 12288)
        
        >>> s = _testingValues[2].binaryString()
        >>> obj = fvb(s, logger=logger)
        test.device - DEBUG - Walker has 12 remaining bytes.
        test.device - DEBUG - Start size is 12 and end size is 16.
        test.device - DEBUG - Format is 3.
        ...
        if format == 0x8000:
            ...
            # DeltaSetInnerIndex; make a VariationIndex and return. Calling
            # code needs to translate DeltaSetIndices to LivingDeltas.
            logger.debug(('Vxxxx', (startSize, endSize), "VariationIndex (%d, %d)"))
            dsi = variationindex.VariationIndex((startSize, endSize))
            return dsi
        ...
        logger.debug((
          'V0095',
          (startSize, endSize),
          "Start size is %d and end size is %d."))
        
        if format not in {1, 2, 3}:
            logger.error(('E5200', (format,), "Unknown format: 0x%04X."))
            return None
        
        logger.debug(('V0094', (format,), "Format is %d."))
