...
"""
Support for Ratio objects for version 0 'VDMX' tables.
"""
...
class CharSet(int, metaclass=enummeta.FontDataMetaclass):
    """
    Objects representing a character set for a version 0 'VDMX' table. This is
    a value of zero or one. A value of zero means no subset is preset (i.e. the
    values apply to all glyphs in the font). A value of one means the VDMX
    group(s) are only computed using the glyphs needed for the Windows ANSI
    character set.
    """
...
class Ratio(object, metaclass=simplemeta.FontDataMetaclass):
    ...
    def fromvalidatedwalker(cls, w, **kwArgs):
        ...
        if logger is None:
            logger = logging.getLogger().getChild('Ratio_v0')
        else:
            logger = logger.getChild('Ratio_v0')
