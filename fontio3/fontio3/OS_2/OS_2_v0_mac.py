...
"""
Support for original Version 0 OS/2 tables, as defined when the original
TrueType specification was released. Note this differs from Microsoft's
definition of Version 0; see OS_2_v0.py for that format.
"""
...
def _merge_version(obj1, obj2, **kwArgs):
    """
    Merges the version attribute. Note that this really does one thing: check
    that the versions for obj1 and obj2 are the same. If they are not, a
    ValueError is raised.
    """
    
    if (obj1 == 0) and (not hasattr(kwArgs['selfParent'], 'sTypoAscender')):
        obj1 = -1
    
    if (obj2 == 0) and (not hasattr(kwArgs['otherParent'], 'sTypoAscender')):
        obj2 = -1
    
    if obj1 != obj2:
        raise ValueError("Cannot merge different OS/2 versions!")
...
def _validate_usWeightClass(value, **kwArgs):
    ...
    if q == 0 and 1 <= r <= 9:  # old-school 1-9; only allow this for v0_mac.
        logger.warning((
          'V1057',
          (value,),
          "The usWeightClass value %d uses the ancient "
          "Apple definition of values in the range 1-9. "
          "This font should probably be updated."))
...
class OS_2(object, metaclass=simplemeta.FontDataMetaclass):
    """
    Objects representing OS/2 tables, version 0 (as defined in the original
    ...
    attrSpec = dict(
        version = dict(
            ...
            attr_mergefunc = _merge_version,
            attr_pprintfunc = (
              lambda p, n, label, **k:
              p.simple("0 (Mac)", label))),
        
        xAvgCharWidth = dict(
            attr_initfunc = (lambda: 0),
            attr_inputcheckfunc = utilities.inRangeShort,
            attr_label = "Average weighted escapement",
            # no mergefunc, because value is recalculated
    ...
    def buildBinary(self, w, **kwArgs):
        ...
        w.add("2H", self.usFirstCharIndex, self.usLastCharIndex)
